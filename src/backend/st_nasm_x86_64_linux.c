...
static void ST_generate_inst(FILE *out, ST_gen_ctx_t *ctx, ST_ir_inst_t *in) {
    ...
    case ST_IR_CALL: {
        ...
        if (rc > 2) {
            ...
            if (n + 1 > ST_N_ARG_REGS)
                ST_todo("too many arguments alongside hidden return pointer");
    ...
    case ST_IR_CALL_INDIRECT: {
        u32 int_id = 0, float_id = 0;
        ST_forrange(0, in->call_ind.args.count) {
            ST_ir_inst_t *arg = in->call_ind.args.items[i];
            if (arg->ty && ST_ty_is_float(arg->ty)) {
                if (float_id >= ST_N_XMM_REGS)
                    ST_todo("Too many floats man");
                ST_fload(out, xmm_regs[float_id], arg);
                float_id++;
            } else {
                if (int_id >= ST_N_ARG_REGS)
                    ST_todo("Too many arguments man");
                ST_load(out, arg_regs[int_id], arg);
                int_id++;
            }
            fprintf(out, "    mov al, %u\n", float_id);
...
static void ST_generate_phi_copies(FILE *out, ST_ir_block_t *from,
                                   ST_ir_block_t *to) {
...
static void ST_generate_term(FILE *out, ST_gen_ctx_t *ctx, ST_ir_block_t *b) {
    ...
    case ST_IR_TERM_RET: {
        if (t->rets.count > 2) {
            ...
            if (t->rets.count >= 1) {
                if (t->rets.items[0]->ty &&
                    ST_ty_is_float(t->rets.items[0]->ty))
                    ST_fload(out, "xmm0", t->rets.items[0]);
                else
                    ST_load(out, "rax", t->rets.items[0]);
            }
            if (t->rets.count >= 2) {
                if (t->rets.items[1]->ty &&
                    ST_ty_is_float(t->rets.items[1]->ty))
