...
static void st_append_ld(ST_arena_t *arena, ST_procs_t *procs, const char *out_path, const char *obj_path,
                         strings_t *ldflags) {
    ST_proc_opt_t opt = {0};
#if defined(__linux__)
    u32 n = 0;
    ...
    opt.args[n++] = obj_path;
    opt.args[n++] = "/ENTRY:_start";
    opt.args[n++] = "/SUBSYSTEM:CONSOLE";

    ST_sb_t sb = {0};
    ST_arena_append_to_builder(arena, &sb, "/OUT:");
    ST_arena_append_to_builder(arena, &sb, out_path);
...
static b8 st_compile(ST_arena_t *arena, const char *path, st_stage_t stage, const char *output,
    ...
    // TODO(krypton): mkdir .build if not exist
    const char *asm_path = stage == ST_STAGE_ASM ? (output ? output : "test.asm")
                                                  : ST_BUILD_DIR "/storthc_build.asm";
    ...
    // TODO(krypton): if os == win32, add .exe to the end if not exist
    const char *exe_path = output ? output : "test" ST_EXE_EXT;
    st_append_ld(arena, &procs, exe_path, obj_path, ldflags);
    if (!ST_run_processes(&procs))
        goto done;
