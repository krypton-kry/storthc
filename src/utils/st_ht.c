...
void ST_ht_init(ST_arena_t *arena, ST_ht_t *ht, u32 init_cap) {
    ...
    ht->arena = arena;
    ht->slots = ST_arena_push_zeroed(arena, sizeof(*ht->slots) * capacity);
...
static b8 ST_key_equal(const ST_ht_generic_t *key1,
                       const ST_ht_generic_t *key2) {
...
static u32 ST_probe_slot(const ST_ht_generic_t *key, u32 level, u32 probe,
                         u32 level_size) {
...
static u32 ST_find_slot(ST_ht_t *ht, const ST_ht_generic_t *key) {
    ...
    for (u32 i = 0; i < nlevels; i++) {
        for (u32 j = 0; j < levels[i].size; j++) {
            u32 slot =
                levels[i].start + ST_probe_slot(key, i, j, levels[i].size);
            ST_ht_generic_t *k = ht->slots[slot].key;
            if (k == NULL)
                break;
            if (k == ST_TOMBSTONE)
                continue;
            if (ST_key_equal(k, key))
                return slot;
        }
    }
...
static u32 st_count_free(ST_ht_t *ht, ST_level_t lvl) {
    ...
    ST_forrange(0, lvl.size) {
        if (ST_slot_free(ht->slots[lvl.start + i].key))
            free_count++;
    }
    return free_count;
}

static b8 st_try_place(ST_ht_t *ht, ST_level_t lvl, ST_ht_generic_t *key,
                       ST_ht_generic_t value, u32 level_index, u32 budget) {
    if (budget > lvl.size)
        budget = lvl.size;
...
void ST_ht_set(ST_ht_t *ht, ST_ht_generic_t *key, ST_ht_generic_t value) {
    ...
    ST_assert(ht != NULL && ht->arena != NULL &&
              "ST_ht_init must be called before ST_ht_set");
    if (!key)
        return;
    ...
    if ((f64)(ht->count + 1) > (f64)ht->capacity * (1.0 - ST_DELTA))
        ST_ht_grow(ht);
    ...
    ST_forrange(0, nlevels) {
        ...
        f64 eps2 = has_next ? (f64)st_count_free(ht, levels[i + 1]) /
                                  (f64)levels[i + 1].size
                            : 0.0;

        if (has_next && eps1 > ST_DELTA / 2.0 && eps2 > 0.25) {
            u32 budget = ST_probe_budget(eps1);
            if (st_try_place(ht, levels[i], key, value, i, budget))
                return;
            if (st_try_place(ht, levels[i + 1], key, value, i + 1,
                             levels[i + 1].size))
                return;
            continue;
            ...
            if (has_next) {
                ...
                if (st_try_place(ht, levels[i], key, value, i, levels[i].size))
                    return;
            }
            continue;
        } else {
