...
static ST_flag_t *ST_flag_push(ST_flag_parser_t *fp, ST_flag_kind_t kind,
                               const char *name, const char *desc, void *value,
                               b8 pos, b8 required) {
    ST_assert(fp != NULL);
    ST_assert(name != NULL);
...
void ST_flag_bool(ST_flag_parser_t *fp, const char *name,
                  const char *description, b8 *value) {
    ST_flag_push(fp, ST_FLAG_BOOL, name, description, value, 0, 0);
}

void ST_flag_int(ST_flag_parser_t *fp, const char *name,
                 const char *description, i32 *value) {
    ST_flag_push(fp, ST_FLAG_INT, name, description, value, 0, 0);
}

void ST_flag_string(ST_flag_parser_t *fp, const char *name,
                    const char *description, char **value) {
    ST_flag_push(fp, ST_FLAG_STRING, name, description, value, 0, 0);
}

void ST_flag_positional(ST_flag_parser_t *fp, const char *name,
                        const char *description, char **value, b8 required) {
    ST_flag_push(fp, ST_FLAG_STRING, name, description, value, 1, required);
}

void ST_flag_alias(ST_flag_parser_t *fp, const char *long_name,
                   char short_name) {
...
static ST_flag_t *ST_flag_find_long(ST_flag_parser_t *fp, const char *name,
                                    u64 len) {
    ST_forrange(0, fp->count) {
        ST_flag_t *flag = &fp->items[i];
...
static b8 ST_flag_set(ST_flag_parser_t *fp, ST_flag_t *flag, const char *value,
    ...
    if (flag->kind == ST_FLAG_BOOL) {
        if (!value)
            *(b8 *)flag->value = 1;
            ...
            fprintf(
                stderr,
                "%s: invalid value '%s' for --%s (expected 0/1/true/false)\n",
...
b8 ST_flag_parse(ST_flag_parser_t *fp, int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!only_positional && strcmp(arg, "--") == 0) {
            only_positional = 1;
        } else if (!only_positional && arg[0] == '-' && arg[1] == '-' &&
                   arg[2]) {
            char *name = arg + 2;
            char *eq = strchr(name, '=');
            ...
            if (!ST_flag_set(fp, flag, eq ? eq + 1 : NULL, &i, argc, argv))
                return 0;
            ...
            if (!ST_flag_set(fp, flag, NULL, &i, argc, argv))
                return 0;
        } else {
...
void ST_flag_usage(ST_flag_parser_t *fp) {
    ...
    ST_forrange(0, fp->count) {
        ...
        char left[64];
        if (flag->alias)
            snprintf(left, sizeof(left), "-%c, --%s%s", flag->alias, flag->name,
                     ST_flag_value_hint(flag->kind));
        else
            snprintf(left, sizeof(left), "    --%s%s", flag->name,
                     ST_flag_value_hint(flag->kind));
...
void ST_flag_dump(ST_flag_parser_t *fp) {
    ST_forrange(0, fp->count) {
        ST_flag_t *flag = &fp->items[i];
        fprintf(stderr, "%-16s pos=%d seen=%d ", flag->name, flag->pos,
                flag->seen);
        switch (flag->kind) {
        case ST_FLAG_BOOL:
